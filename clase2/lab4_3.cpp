...
int main () {
    ...
    #pragma omp target teams distribute parallel for num_teams(512) num_threads(1024) map(tofrom:A[0:N],B[0:N])
    for (int i = 1; i < N-1; i++)
        f(A,B,i);
    ...
    #pragma omp target teams distribute parallel for num_teams(512) num_threads(1024) 
    for (int i = 1; i < N-1; i++)
        f(A,B,i);
